...
async def q_3(message: types.Message, state: FSMContext):
    mystate = await state.get_data()
    #функционал выберет при одних и тех же ответах одно и то же животное, при расширении списка
    #животных в animals.py можно внедрить выбор случайного животного из ограниченных ответами на вопросы списков
...
async def a_2(message: types.Message, state: FSMContext):
    ...
    if message.text == "Написать отзыв":
        ...
        if dict_of_animals[mystate['Parametr']][0] == "Обыкновенный шакал" or dict_of_animals[mystate['Parametr']][0] == "Японский макак":
            ...
            # lines_with_i = Text_of_animal.find_all('i')
            # for i in lines_with_i:
            #     bot_list_of_msg.append(i.text)
            # lines_with_text = Text_of_animal.find_all('text-align: justify;')
            # for text in lines_with_text:
            #     bot_list_of_msg.append(text.text)
